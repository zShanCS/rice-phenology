...
function generateFilterData(mosaic_data, filter) {
    let data = [];
    Object.keys(mosaic_data).forEach(field => {
        Object.keys(mosaic_data[field]).forEach(date => {
            let ndvi = mosaic_data[field][date][filter];
            data.push({
                field: field,
                date: date,
                ndvi: ndvi
...
export default generateTimeTakenData;
